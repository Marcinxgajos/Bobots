...
public class NearestPointFromPolygon : MonoBehaviour {
   ...
   // public List<Transform> trLst;
	...
	void Start ()
    {
        ...
        if(polygon == null)
        {
            polygon = GameObject.Find("PolygonSimply").transform;
        }
      ...
      //  Debug.Break();
        ...
        */
        ...
        while (intersectionLineSeg.Count > 0)
        {
            ...
            lineSegsOuter.Sort((x, y) => x.angle.CompareTo(y.angle));


            

          //  lineSegsOuter.Sort((x, y) => y.angle.CompareTo(x.angle));
            //lineSegsOuter.OrderByDescending(x => x.angle).ToList();
            //try to change to OrderBy
            //  lineSegsOuter.OrderBy(x => x.angle).ToList();
        ...
        //trzeba zliczac ile jest dziurek.

        bool []IsToRemove;
        IsToRemove = new bool[lst.Count];
        Array.Clear(IsToRemove, 0, IsToRemove.Length);
        ...
        {
            if(!IsToRemove[i]  && IsToRemove[(i+1)%lst.Count] )
            {
                //zalozmy ze i++
                i++;
                numberOfHoles++;
                while(IsToRemove[i%lst.Count] && i<lst.Count+2)
                {
                    i++;
                }
        ...
        {
            if (IsToRemove[i])
                debugToRemove = debugToRemove + "X";
            else
                debugToRemove = debugToRemove + "_";
            idxsString = idxsString + lst[i].Value.Index.ToString() + " ";
        }
        ...
        //todo generate separate polygon.
